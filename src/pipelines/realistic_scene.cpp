...
Prism::SkyboxDrawcall::SkyboxPipeline::SkyboxPipeline(PrismApp* app) : Pipeline(app->GetDevice().GetComPtr()), app_(app) {
    std::vector<D3D12_INPUT_ELEMENT_DESC> layout;
    for (auto& l : VertexAttrs::MakeInputElements()) {
        layout.push_back(l);
    }
    ...
    pso_desc_.SampleDesc = GetSampleDesc(app_->GetInitializeParams().msaa_type);
    auto [vs_ptr, vs_len] = app_->GetShaderLoader().LoadShader("skybox", ShaderType::VertexShader).value();
    auto [ps_ptr, ps_len] = app_->GetShaderLoader().LoadShader("skybox", ShaderType::PixelShader).value();
    pso_desc_.VS = {vs_ptr, vs_len};
    pso_desc_.PS = {ps_ptr, ps_len};
    pso_desc_.NumRenderTargets = 1;
    pso_desc_.RTVFormats[0] = app_->GetInitializeParams().rt_format;
