...
namespace Prism
{
    ...
    class ResourceManager {
        ...
        DXGI_FORMAT ParseGLTFImageFormat(const tinygltf::Image& img) {
            ...
            } else if (img.pixel_type == TINYGLTF_COMPONENT_TYPE_FLOAT) {
                switch (img.component) {
    ...
    public:
        ...
        template<typename T>
        ResourceHandle CreateLocalBuffer(const std::string& name, const std::vector<T>& vector, D3D12_RESOURCE_STATES initial_states, D3D12_RESOURCE_FLAGS flags = D3D12_RESOURCE_FLAG_NONE, D3D12_CLEAR_VALUE* pclr = nullptr, bool single_layer = false) {
        ...
        ResourceHandle CreateTexture2DFromGLTFImage(const std::string& name, const tinygltf::Image& img) {
            ResourceHandle texture = CreateTexture2D(name, img.width, img.height, 1, ParseGLTFImageFormat(img), D3D12_RESOURCE_STATE_COMMON);
            auto tex_desc = texture->GetD3D12Resource()->GetDesc();
            D3D12_PLACED_SUBRESOURCE_FOOTPRINT footprint;
            uint32_t rows;
            uint64_t row_pitch;
            ...
            device_->GetCopyableFootprints(&tex_desc, 0, 1, 0, &footprint, &rows, &row_pitch, &total_bytes);
            void* mapping;
            ResourceHandle upload = CreateLocalBuffer("UploadBuffer_" + name, 1, total_bytes, D3D12_RESOURCE_STATE_COMMON, D3D12_RESOURCE_FLAG_NONE, nullptr, true);
            upload->GetD3D12Resource()->Map(0, nullptr, &mapping);
            memset(mapping, 0, total_bytes);
            auto* dest = static_cast<uint8_t *>(mapping) + footprint.Offset;
            uint64_t aligned_dest_row_pitch = footprint.Footprint.RowPitch;
            uint64_t dest_pitch = row_pitch / footprint.Footprint.Width;
            uint64_t src_pitch = img.component * img.bits / 8;
            uint64_t src_row_pitch = src_pitch * img.width;
            for (int j = 0; j < img.height; ++j) {
                for (int i = 0; i < img.width; ++i) {
                    for (int a = 0; a < src_pitch; ++a) {
                        dest[j * aligned_dest_row_pitch + i * dest_pitch + a] = img.image[j * src_row_pitch + i * src_pitch + a];
                    }
                }
            }
            upload->GetD3D12Resource()->Unmap(0, nullptr);
