...
namespace Prism
{
    class RenderContext {
        ...
        DescriptorHeap<D3D12_DESCRIPTOR_HEAP_TYPE_DSV> shadow_heap_;

        ComPtr<IDXGISwapChain4> swapchain_;
        uint64_t index_ {};
    public:
        RenderContext(const dx_init_t& init, HWND hwnd, Device& device, ResourceManager& rm, RecordDispatcher& render_dispatcher) : init_(init), device_(device.GetComPtr()), mgr_(rm), render_dispatcher_(render_dispatcher), rtv_heap_(device_, init.buffer_count), dsv_heap_(device_, init.buffer_count), msaa_heap_(device_, 1), shadow_heap_(device_, init.buffer_count) {
            ...
            if (init.msaa_type != MSAAType::NONE) {
                msaa_buffer = mgr_.CreateTexture2D(msaa_buffer_name, init.width, init.height, 1, init.rt_format, DXGI_FORMAT_UNKNOWN, D3D12_RESOURCE_STATE_COMMON, GetSampleDesc(init_.msaa_type), false, D3D12_RESOURCE_FLAG_ALLOW_RENDER_TARGET, &rt_clr);
            ...
            for (int i = 0; i < init.buffer_count; ++i) {
                ...
                ResourceHandle depth_buffer = mgr_.CreateTexture2D(depth_buffer_name, init.width, init.height, 1, init.ds_format, DXGI_FORMAT_UNKNOWN, D3D12_RESOURCE_STATE_DEPTH_WRITE, GetSampleDesc(init_.msaa_type), false, D3D12_RESOURCE_FLAG_ALLOW_DEPTH_STENCIL, &depth_clr);
                ...
                ResourceHandle shadow_map = mgr_.CreateTexture2D(shadow_map_name, init.width, init.height, 1, DXGI_FORMAT_D32_FLOAT, DXGI_FORMAT_R32_FLOAT, D3D12_RESOURCE_STATE_DEPTH_WRITE, { 1, 0 }, true, D3D12_RESOURCE_FLAG_ALLOW_DEPTH_STENCIL, &depth_clr);
                device_->CreateDepthStencilView(shadow_map->GetD3D12Resource(), nullptr, shadow_heap_.GetCPUHandle(i));
                rts_.emplace_back(init_, back_buffer, depth_buffer, msaa_buffer, shadow_map, rtv_heap_.GetCPUHandle(i), dsv_heap_.GetCPUHandle(i), mrtv_handle, shadow_heap_.GetCPUHandle(i));
            }
        }

        void Shadow(std::initializer_list<RecordDispatcher::GPUProcess> processes) {
            std::vector<RecordDispatcher::GPUProcess> all;
            all.reserve(processes.size() + 2);
            RenderTarget& rt = rts_[index_];
            all.push_back(rt.BeginShadow());
            for (size_t i = 0; i < processes.size(); ++i) {
                all.push_back(processes.data()[i]);
            }
            all.push_back(rt.FinishShadow());
